package org.debugroom.sample.domain.service.mybatis;
 ...
 */
@Service
public class Mybatis2SampleService implements AnnotatedService {

	private static final Logger logger = LoggerFactory.getLogger(Mybatis2SampleService.class);
	
	@Autowired
	@Qualifier("userRepositoryMybatis2Impl")
	...
	public void execute() {
		...
		addUser.setUserId(StringUtils.right(
				(((new StringBuffer()).append("00000000")
										.append(userRepository.getNumberOfUser()))
												.toString()), 8));
		addUser.setUserName("AddedByMybatisSampleService");
		...
		addUser.setUserName("UpdatedByMybatisService");
